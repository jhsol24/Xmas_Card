...
public class WNSearch {
	...
	private int search(String query, boolean isRealQuery, String[] userInfo, int hiSum, int useOrResult, int connectionOpt, boolean useSuggestedQuery) {
		...
		//Connection
		ret = common.getConnection(wncol.SEARCH_IP, wncol.SEARCH_PORT, WNCollection.CONNECTION_TIMEOUT);
	...
	public int setCollectionBasicInfo(int idx) {
		int ret = 0;
		String[] analyzer = WNUtils.split(wncol.COLLECTION_INFO[idx][WNCollection.ANALYZER], ",");
		String[] pageInfo = WNUtils.split(wncol.COLLECTION_INFO[idx][WNCollection.PAGE_INFO], ",");
		String[] rankingOption = WNUtils.split(wncol.COLLECTION_INFO[idx][WNCollection.RANKING_OPTION], ",");
		String sortFieldInfo = wncol.COLLECTION_INFO[idx][WNCollection.SORT_FIELD];
		if(analyzer.length < 5 || pageInfo.length < 2 
			...
			appendWarnings("[WARNING] [setCollectionBasicInfo] ["+ wncol.COLLECTIONS[idx] 
					+ "] Analyzer : '" + wncol.COLLECTION_INFO[idx][WNCollection.ANALYZER]
							+ "', PageInfo : '" + wncol.COLLECTION_INFO[idx][WNCollection.PAGE_INFO]
									+ "', RankingOption : '"+ wncol.COLLECTION_INFO[idx][WNCollection.RANKING_OPTION]
											+ "' error");
			return -1;
		}
	...
	public int addCollection(int idx, int useKma, int isCase, int useOriginal, int useSynonym, int useDuplicateDetection){
		...
		String colName = wncol.COLLECTIONS_NAME[idx];
		//System.out.println(indexName+"::"+colName);
	...
	public int setSearchFieldInfo(int idx) {
		...
		String searchFields = wncol.COLLECTION_INFO[idx][WNCollection.SEARCH_FIELD];
	...
	public int setCollectionQuery(int idx) {
		int ret = 0;
		//컬렉션 검색 설정
		if (!wncol.COLLECTION_INFO[idx][WNCollection.COLLECTION_QUERY].equals("")) {
			if(isDebug) {
				checkCollectionQueryFieldName("[w3SetCollectionQuery] [setCollectionQuery]", wncol.COLLECTIONS[idx],wncol.COLLECTION_INFO[idx][WNCollection.COLLECTION_QUERY] );
			}
			ret = common.setCollectionQuery(wncol.COLLECTIONS[idx],
					wncol.COLLECTION_INFO[idx][WNCollection.COLLECTION_QUERY]);
		}
		return ret;
	}
	...
	public int setBoostQuery(int idx) {
		int ret = 0;
		//Boost 검색 설정
		if (!wncol.COLLECTION_INFO[idx][WNCollection.BOOST_QUERY].equals("")) {
			/* if(isDebug) {
				checkCollectionQueryFieldName("[w3SetBoostQuery] [setBoostQuery]", COLLECTIONS[idx],wncol.COLLECTION_INFO[idx][COLLECTION_QUERY] );
			} */
			ret = common.setBoostQuery(wncol.COLLECTIONS[idx], wncol.COLLECTION_INFO[idx][WNCollection.BOOST_QUERY]);
		}
		return ret;
	}
	...
	public int setDateRange(int idx) {
		int ret = 0;
		//날짜검색 범위 설정
		if (!wncol.COLLECTION_INFO[idx][WNCollection.DATE_RANGE].equals("")) {
			String[] dateFields = WNUtils.split(wncol.COLLECTION_INFO[idx][WNCollection.DATE_RANGE], ",");
			if(dateFields.length < 2) {
				appendWarnings("[WARNING] [w3SetDateRange] [setDateRange] [" + wncol.COLLECTIONS[idx] + "] DATE_RANGE : '" + wncol.COLLECTION_INFO[idx][WNCollection.DATE_RANGE] + "' error.");
				return -1;
			}
	...
	public int setRankRange(int idx) {
		int ret = 0;
		//날짜검색 범위 설정
		if (!wncol.COLLECTION_INFO[idx][WNCollection.RANK_RANGE].equals("")) {
			String[] rankFields = WNUtils.split(wncol.COLLECTION_INFO[idx][WNCollection.RANK_RANGE], ",");
			if(rankFields.length < 2) {
				appendWarnings("[WARNING] [w3SetRankRange] [setRankRange] [" + wncol.COLLECTIONS[idx] + "] RANK_RANGE : '" + wncol.COLLECTION_INFO[idx][WNCollection.RANK_RANGE] + "' error.");
				return -1;
			}
	...
	public int setPrefixQuery(int idx){
		int ret = 0;
		//확장 검색 설정
		if (!wncol.COLLECTION_INFO[idx][WNCollection.EXQUERY_FIELD].equals("")) {
			if(isDebug) {
				checkExFieldName("[w3AddExQuery] [setExQuery]", wncol.COLLECTIONS[idx]
						, wncol.COLLECTION_INFO[idx][WNCollection.EXQUERY_FIELD]);
			}
	...
	public int setBoostCategory(int idx){
		int ret = 0;
		//확장 검색 설정
		if (!wncol.COLLECTION_INFO[idx][WNCollection.CATEGORY_BOOST].equals("")) {
			String[] dataFields = WNUtils.split(wncol.COLLECTION_INFO[idx][WNCollection.CATEGORY_BOOST], ",");
	...
	public int addCategoryGroupBy(int idx){
		...
		if (!wncol.COLLECTION_INFO[idx][WNCollection.CATEGORY_GROUPBY].equals("")) {
			String[] categoryGroup = WNUtils.split(wncol.COLLECTION_INFO[idx][WNCollection.CATEGORY_GROUPBY], ",");
	...
	public int addCategoryQuery(int idx){
		...
		if (!wncol.COLLECTION_INFO[idx][WNCollection.CATEGORY_QUERY].equals("")) {
			String[] categoryQuery = WNUtils.split(wncol.COLLECTION_INFO[idx][WNCollection.CATEGORY_QUERY], ",");
	...
	public int setAuthorityQuery(int idx) {
		int ret = 0;
		//컬렉션 검색 설정
		if (!wncol.COLLECTION_INFO[idx][WNCollection.AUTH_QUERY].equals("")) {
			String[] dataFields = WNUtils.split(wncol.COLLECTION_INFO[idx][WNCollection.AUTH_QUERY], ",");
	...
	public int setPropertyGroup(int idx) {
		int ret = 0;
		//컬렉션 검색 설정
		if (!wncol.COLLECTION_INFO[idx][WNCollection.PROPERTY_GROUP].equals("")) {
			String[] dataFields = WNUtils.split(wncol.COLLECTION_INFO[idx][WNCollection.PROPERTY_GROUP], ",");
	...
	public int setFilterQuery(int idx) {
		int ret = 0;
		if (!wncol.COLLECTION_INFO[idx][WNCollection.FILTER_OPERATION].equals("")) {
			if(isDebug) {
				checkFilterFieldName("[w3SetFilterQuery] [setFilterQuery]", wncol.COLLECTIONS[idx]
						, wncol.COLLECTION_INFO[idx][WNCollection.FILTER_OPERATION]);
			}
			ret = common.setFilterQuery(wncol.COLLECTIONS[idx], wncol.COLLECTION_INFO[idx][WNCollection.FILTER_OPERATION]);
		}
		return ret;
	}
	...
	public int setMultiGroupBy(int idx) {			
		int ret = 0;
		if (!wncol.COLLECTION_INFO[idx][WNCollection.MULTI_GROUP_BY].equals("")) {
			String field = wncol.COLLECTION_INFO[idx][WNCollection.MULTI_GROUP_BY];
			//그룹전달인자 확인
	...
	public int setGroupBy(int idx) {
		int ret = 0;
		if (!wncol.COLLECTION_INFO[idx][WNCollection.GROUP_BY].equals("")) {
			String[] groupby = WNUtils.split(wncol.COLLECTION_INFO[idx][WNCollection.GROUP_BY], ",");
			if(isDebug) {
			...
			if(groupby.length == 2){
				...
				appendWarnings("[WARNING] [w3SetGroupBy] [setGroupBy] [" + wncol.COLLECTIONS[idx] + "] " +wncol.COLLECTION_INFO[idx][WNCollection.GROUP_BY]+ "  Check variable format and value.");
			}
		}
		return ret;
	}
	...
	public int addSortFieldInGroup(int idx){
		int ret = -1;
		//그룹 정렬 전달인자 확인
		if (wncol.COLLECTION_INFO[idx][WNCollection.GROUP_SORT_FIELD] != ""){
			String sortValue = wncol.COLLECTION_INFO[idx][WNCollection.GROUP_SORT_FIELD];
	...
	public int setDuplicateDetectionCriterionField(String collectionName, int idx){
		...
		int ret = -1;
		if (wncol.COLLECTION_INFO[idx][WNCollection.DEDUP_SORT_FIELD] != ""){
			String sortValue = wncol.COLLECTION_INFO[idx][WNCollection.DEDUP_SORT_FIELD];
	...
	public int setResultFieldInfo(int idx) {
		int ret = 0;
		String resultFields = wncol.COLLECTION_INFO[idx][WNCollection.RESULT_FIELD];
	...
	public String getField(String collectionName, String field, int idx, boolean isHL) {
		...
		if (field.equalsIgnoreCase("UID")) {
		...
		}else if (field.equalsIgnoreCase("RANK")) {
		...
		}else  if (field.equalsIgnoreCase("SEARCHERID")) {
			ret = common.getSearcherId(collectionName, idx);
		}else  if (field.equalsIgnoreCase("COLLECTIONID")) {
			...
			if(isHL){
				ret = getKeywordHl(common.getField(collectionName, field ,idx), "bold");
			}else{
	...
	public int setMergeCollectionBasicnfo(int idx) {
		...
		String mergeCollectionName = wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_COLLECTION_NAME];
		String[] collectionNames = WNUtils.split(wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_MAPPING_COLLECTION_NAME], "/");
		String[] pageInfo = WNUtils.split(wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_PAGE_INFO], ",");
		if(pageInfo.length < 2 ) {
			appendWarnings("[WARNING] [setMergeCollectionBasicnfo] [PageInfo : '" + wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_PAGE_INFO]
					+ "' error");
			return -1;
		}
	...
	public int addMergeDocumentField(int idx) {
		...
		String[] mergeFields = WNUtils.split(wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_RESULT_FIELD], ",");
		int mergeFieldLength = mergeFields.length;

		String[][] documentFields = new String[collectionLength][mergeFieldLength];
		String[] temp = WNUtils.split(wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_MAPPING_RESULT_FIELD], "/");
	...
	public int addMergeMultiGroupBy(int idx) {
		int ret = 0;
		if (!wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_MULTI_GROUP_BY_FIELD].equals("")) {
			String mergeCollectionName = wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_COLLECTION_NAME];
			String[] collectionNames = WNUtils.split(wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_MAPPING_COLLECTION_NAME], "/");
			...
			String[] mergeMultiGroupByFields = WNUtils.split(wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_MULTI_GROUP_BY_FIELD], ",");
			int mergeLength = mergeMultiGroupByFields.length;

			String[][] documentFields = new String[collectionLength][mergeLength];
			String[] temp = WNUtils.split(wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_MAPPING_MULTI_GROUP_BY_FIELD], "/");
	...
	public int addMergeCategoryGroupByField(int idx) {
		int ret = 0;
		if (!wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_CATEGORY_GROUPBY_FIELD].equals("")) {
			...
			String[] mergeCategoryGroupByFields = WNUtils.split(wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_CATEGORY_GROUPBY_FIELD], ",");
			...
			String[] temp = WNUtils.split(wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_MAPPING_CATEGORY_GROUPBY_FIELD], "/");
	 ...
	 */
	public String getKeywordHl(String content, String style) {
		if(content != null) {
			content = content.replaceAll("<!HS>", "<span class=\"blue3\">");
			content = content.replaceAll("<!HE>", "</span>");
		}
		return content;
	}
	...
	public int checkValidDocumentField(String collectionName, String field, int item) { 
		...
		if(idx != -1){ 
			...
			if(idx != -1) { 
				String[] fields = WNUtils.split(wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_RESULT_FIELD], ",");
				int length = fields.length; 
				for(int i=0; i<length; i++) { 
				...
				if (chk == -1) { 
					String[] fields1 = WNUtils.split(wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_CATEGORY_GROUPBY_FIELD], ",");
	...
	public int checkSortFieldName(String collectionName, String sortField, int sortOrder) {
		...
		} else if(sortField.equalsIgnoreCase("RANK")) {
			if(sortOrder != 1) {
		...
		}else if(sortField.equalsIgnoreCase("WEIGHT")) {
			if(sortOrder != 1 && sortOrder != 0) {
		...
		}else if(sortField.equalsIgnoreCase("UID")) {
			if(sortOrder != 1 && sortOrder != 0) {
				//UID는 오름차순(0)과 내림차순(1)으로만 정렬 가능
				appendWarnings("[WARNING] [w3AddSortField] [checkSortFieldName] [" + collectionName + "] UID sortOrder '" + sortOrder +"' Only 0 or 1");
				return -1;
			}
		}else {
			// 정렬 필드 검사
	...
	public void debugMsg(boolean isRealQuery, String query){
		sb.append("[CHARSET] " + WNCollection.CHARSET + " [QUERY] " + query);
		sb.append("<br>[w3ConnectServer] [IP] " + wncol.SEARCH_IP + " [PORT] " + wncol.SEARCH_PORT + " [TIMEOUT]" + WNCollection.CONNECTION_TIMEOUT);
		if(!isUidSrch) {
			sb.append("<br>[w3RecvResult] Mode : " + this.connectionOpt);
			if(isRealQuery){
				sb.append("<br>[W3ReceiveRecentQueryListResult] Set Count : " + WNCollection.REALTIME_COUNT);
			}
		}
		...
		for(int i=0; i < collections.length; i++) {
			...
			// 컬렉션 정보
			String content = wncol.COLLECTION_INFO[idx][WNCollection.INDEX_NAME];
			if(content != null && !content.equals("")){
				sb.append("<br>[w3AddCollection] <b>" + content + "</b>");
			}
			if(!isUidSrch) {
				//페이지 설정
				content = wncol.COLLECTION_INFO[idx][WNCollection.PAGE_INFO];
				if(content != null && !content.equals("")){
					sb.append("<br>[w3SetPageInfo] " + content);
				}

				//분석 정보
				content = wncol.COLLECTION_INFO[idx][WNCollection.ANALYZER];
				if(content != null && !content.equals("")){
				...
				//정렬 정보
				content = wncol.COLLECTION_INFO[idx][WNCollection.SORT_FIELD];
				if(content != null && !content.equals("")){
					sb.append("<br>[w3AddSortField] " + content);
				}

				//검색 필드 정보
				String search_field = wncol.COLLECTION_INFO[idx][WNCollection.SEARCH_FIELD];
				String[] searchField = search_field.split(",");

				content = wncol.COLLECTION_INFO[idx][WNCollection.SEARCH_FIELD];
				if(content != null && !content.equals("")){
				...
				//기간 정보
				content = wncol.COLLECTION_INFO[idx][WNCollection.DATE_RANGE];
				if(content != null && !content.equals("")){
					sb.append("<br>[w3SetDateRange] " + content);
				}

				//확장 검색  정보
				content = wncol.COLLECTION_INFO[idx][WNCollection.EXQUERY_FIELD];
				if(content != null && !content.equals("")){
					sb.append("<br>[w3SetPrefixQuery] " + content.replaceAll("<", "&lt;").replaceAll(">", "&gt;"));
				}

				//카테고리 부스트 정보
				content = wncol.COLLECTION_INFO[idx][WNCollection.CATEGORY_BOOST];
				if(content != null && !content.equals("")){
					sb.append("<br>[w3SetBoostCategory] " + content);
				}

				//컬렉션 쿼리 정보
				content = wncol.COLLECTION_INFO[idx][WNCollection.COLLECTION_QUERY];
				if(content != null && !content.equals("")){
					sb.append("<br>[w3SetCollectionQuery] " + content.replaceAll("<", "&lt;").replaceAll(">", "&gt;"));
				}

				//필터 정보
				content = wncol.COLLECTION_INFO[idx][WNCollection.FILTER_OPERATION];
				if(content != null && !content.equals("")){
					sb.append("<br>[w3SetFilterQuery] " + content.replaceAll("<", "&lt;").replaceAll(">", "&gt;"));
				}

				//그룹화  정보
				content = wncol.COLLECTION_INFO[idx][WNCollection.GROUP_BY];
				if(content != null && !content.equals("")){
					sb.append("<br>[w3SetGroupBy] " + content);
				}

				//그룹 정렬  정보
				content = wncol.COLLECTION_INFO[idx][WNCollection.GROUP_SORT_FIELD];
				if(content != null && !content.equals("")){
			...
			//결과 필드 정보
			content = wncol.COLLECTION_INFO[idx][WNCollection.RESULT_FIELD];
			if(content != null && !content.equals("")){
		...
		if(mergeCollections != null) {
			for(int i=0; i < mergeCollections.length; i++) {
				...
				String content = wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_COLLECTION_NAME];
				String mappingColl =  wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_MAPPING_COLLECTION_NAME];
				...
				content = wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_PAGE_INFO];
				if(content != null && !content.equals("")){
					sb.append("<br>[w3SetMergePageInfo] " + content);
				}

				content = wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_RESULT_FIELD];
				String mappingDocs = wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_MAPPING_RESULT_FIELD];
				...
				content = wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_MULTI_GROUP_BY_FIELD];
				String mappingMulti = wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_MAPPING_MULTI_GROUP_BY_FIELD];
				...
				content = wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_CATEGORY_GROUPBY_FIELD];
				String mappingCategory = wncol.MERGE_COLLECTION_INFO[idx][WNCollection.MERGE_MAPPING_CATEGORY_GROUPBY_FIELD];
	...
	public String getPageLinks(int startCount, int totalCount, int viewListCount, int bundleCount) {
		...
		//FIXME 페이징처리 디자인 추가 2020_12_10
		sbRet.append("<div class=\"btn_num\">");
//getCurPageNumber: 현재 페이지 
		if(wnanchor.getCurPageNumber() > 1 ){
			//sbRet.append("<a class=\"img\" href=\"#none\" onClick=\"javascript:pagingProcessing('"+0+"');\" title=\"첫페이지\"><img src=\"/images/ggc/search/s_first.gif\" alt=\"맨처음\"></a> ");
			sbRet.append("<a class=\"num\" href=\"#none\" onclick=\"javascript:pagingProcessing('"+0+"');\" title=\"첫 페이지\">&lt;&lt;</a>");
		} else {
			//sbRet.append("<a href=\"#none\" title=\"첫페이지\"><img src=\"\" alt=\"맨처음\"></a> ");
			sbRet.append("<a class=\"num\" href=\"#none\" title=\"첫 페이지\">&lt;&lt;</a>");
		}
		
		if((wnanchor.getCurPageNumber() -1 )/10 > 0){
			//sbRet.append("<a class=\"img\" href=\"#none\" onClick=\"javascript:pagingProcessing('"+( ((wnanchor.getCurPageNumber()-1)/10-1) *100 )+"');\" class=\"img\"><img src=\"/images/ggc/search/s_prev.gif\" alt=\"이전\"></a> ");
			sbRet.append("<a class=\"num\" href=\"#none\" onclick=\"javascript:pagingProcessing('"+( ((wnanchor.getCurPageNumber()-1)/10-1) *100 )+"');\" title=\"이전 페이지\">&lt;</a>");
		} else {
			//sbRet.append("<a class=\"img\" href=\"#none\" class=\"img\"><img src=\"/images/ggc/search/s_prev.gif\" alt=\"이전\"></a> ");
			sbRet.append("<a class=\"num\" href=\"#none\" title=\"이전 페이지\">&lt;</a>");
		}
		...
		for(int i=0; i<pageCount && i < pages.length; i++) {
			if(pages[i][1].equals("-1")) {
				sbRet.append("<a class=\"num click\" href=\"#none\" title=\"현재 페이지\"> "+pages[i][0]+" </a>");
			} else {
				sbRet.append("<a href=\"#none\" onclick=\"javascript:pagingProcessing('"+pages[i][1]+"');\"> "+pages[i][0]+" </a>");
			}
		}

		if( wnanchor.getTotalPgCount() != wnanchor.getCurPageNumber()
				&& (wnanchor.getTotalPgCount()/10) > ((wnanchor.getCurPageNumber()-1)/10)){
			//sbRet.append("<a href=\"#none\" onClick=\"javascript:pagingProcessing('"+ ( (wnanchor.getCurPageNumber()-1)/10 == 0 ? 1*100 : ((wnanchor.getCurPageNumber()-1)/10 +1)*100  ) +"');\" class=\"img\"><img src=\"/images/ggc/search/s_next.gif\" alt=\"다음\"></a> ");
			sbRet.append("<a class=\"num\" href=\"#none\" onclick=\"javascript:pagingProcessing('" + ((wnanchor.getCurPageNumber()-1)/10 == 0 ? 1*100 : ((wnanchor.getCurPageNumber()-1)/10 +1)*100) + "');\" title=\"다음 페이지\">&gt;</a>");
		} else {
			//sbRet.append("<a href=\"#none\" class=\"img\"><img src=\"/images/ggc/search/s_next.gif\" alt=\"다음\"></a> ");
			sbRet.append("<a class=\"num\" href=\"#none\" title=\"다음 페이지\">&gt;</a> ");
		}

		if(wnanchor.getCurPageNumber() < wnanchor.getTotalPgCount()) {
			//sbRet.append("<a class=\"img\" href=\"#none\" onClick=\"javascript:pagingProcessing('"+(wnanchor.getTotalPgCount()-1)*10+"')\" title=\"마지막페이지\"><img src=\"/images/ggc/search/s_last.gif\" alt=\"마지막\"></a>");
			sbRet.append("<a class=\"num\" href=\"#none\" onclick=\"javascript:pagingProcessing('"+(wnanchor.getTotalPgCount()-1)*10+"')\" title=\"마지막 페이지\">&gt;&gt;</a>");
		} else {
			//sbRet.append("<a class=\"img\" href=\"#none\" title=\"마지막페이지\"><img src=\"/images/ggc/search/s_last.gif\" alt=\"마지막\"></a>");
			sbRet.append("<a class=\"num\" href=\"#none\" title=\"마지막 페이지\">&gt;&gt;</a>");
		}
	...
	*/
	...
	//2021.06.29 _js 추가 cause : javascript pagingProcessing x
	public String getPageLinks(int startCount, int totalCount, int viewListCount, int bundleCount) {
		StringBuffer sbRet = new StringBuffer();
		//wnanchor 정보가 담겨 있습니다 .
		WNAnchor wnanchor = getPageAnchor(startCount, totalCount, viewListCount, bundleCount);
		sbRet.append("<div class=\"btn_num\">");
		...
		for(int i=0; i<pageCount && i < pages.length; i++) {
			if(pages[i][1].equals("-1")) {
				sbRet.append("<strong  class=\"num click \">" + pages[i][0] + "</strong>");
			} else {
	...
	public String getCollectionKorName(String col) {
		...
		for ( int idx=0;idx< wncol.COLLECTIONS.length; idx++){
			String coll = wncol.COLLECTION_INFO[idx][WNCollection.COLLECTION_NAME].trim();
			if ( coll.equals(col) )
				return wncol.COLLECTION_INFO[idx][WNCollection.COLLECTION_KOR];
	...
	public String getCollectionBasicInfo(String col, String info) {
		String value = "";
		int flag = -1;
		for(int i=0; i < wncol.COLLECTIONS_BASIC_INFO.length; i++){
			if(info.equals(wncol.COLLECTIONS_BASIC_INFO[i])) flag = i;
		}
		if(flag == WNCollection.COLLECTION_COUNT){
			value = ""+getResultTotalCount(col);
		} else if(flag == WNCollection.RESULT_COUNT){
			value = ""+getResultCount(col);
		} else {
			value = "-999";
		}
		return value;
	...
	public void setSearcherInfo(String ip, int port){
		wncol.SEARCH_IP = ip;
		wncol.SEARCH_PORT = port;
	}

	public void setManagerInfo(String ip, int port){
		wncol.MANAGER_IP = ip;
		wncol.MANAGER_PORT = port;
	}
}
