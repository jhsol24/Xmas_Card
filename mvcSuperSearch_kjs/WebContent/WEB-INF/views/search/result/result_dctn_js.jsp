...
<body>
  ...
  <c:choose>
				       ...
				       <c:forEach items="${colList.dctn.Document }" var="row" begin="0" end="${colList.TOTAL_VIEW_COUNT }">
                         ...
                         <dl class="resultsty_1">
                            <dt>
                                ...
                                <span>${date }</span>
                                <input type="button"   class="btn_sch"  onClick="javascript:doSearch();" title="유사검색 문서"> </a>
				    		</dt>
                            <dd>${row.TMNL_DEF }</dd>
                            <dd class="cate" style="float: left">${row.TERMS }</dd>
				  ...
				  <c:otherwise>
				  <form name="search_result_dctn" id="search_result_dctn">
