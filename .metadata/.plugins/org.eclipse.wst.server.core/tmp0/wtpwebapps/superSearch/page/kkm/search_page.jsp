<%@ page contentType="text/html; charset=UTF-8"%><%@ include file="./common/api/WNSearch.jsp" %><% request.setCharacterEncoding("UTF-8");%><%
	...
	String ctgr = getCheckReqXSS(request, "ctgr", "");			//dctn 컬렉션 파라미터 문서구분
	
	String strOperation  = "" ;												//operation 조건 필드
    String exquery = "" ;													//exquery 조건 필드
    String dctnExquery = "" ;													//dctn exquery 조건 필드
    ...
    //exquery 설정
    if(!"".equals(ctgr)){
     	dctnExquery = "<CTGR:contains:" + ctgr + ">";
    }
    
    System.out.println("###dctnExquery  : " + dctnExquery );

    String[] collections = null;
    if(collection.equals("ALL")) { //통합검색인 경우
        collections = COLLECTIONS; // ["dctn","doc","low"]
    } else {                        //개별검색인 경우
        collections = new String[] { collection };//[]"dctn"]
    }
    ...
    for (int i = 0; i < collections.length; i++) {
        ...
        if("dctn".equals(collections[i])){
        	wnsearch.setCollectionInfoValue(collections[i], EXQUERY_FIELD, dctnExquery);
        }else{
	        if (!exquery.equals("")) {
	...
	<form name="search" id="search" action="<%=request.getRequestURI()%>" method="post">
		...
		<div id="search_top">
            ...
            <ul>
				...
				<li class="btndsearch">
				문서구분 : 
					<select name="ctgr" onchange="javascript:doSearch();">
						<option value="">전체</option>
						<option value="CT043">수학</option>
						<option value="CT095">영어</option>
						<option value="CT025">국어</option>
						<option value="CT009">과학</option>
					</select>
				</li>
            </ul>
			<!-- ARK search suggest -->
...
<%@ include file="./result/result_low.jsp" %><!-- -->
				<!-- paginate -->
